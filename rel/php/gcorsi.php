...
<body>
	...
	<div id="content"> 
		...
		//Se l'amministratore ha scelto di creare un nuovo corso mostro il form corrispondente
			...
			echo '<tr><td>Inserisci nome Corso:</td><td><input type="text" value="" name="nomecorso"></td></tr>';
			echo '<tr><td>Inserisci Livello :</td><td> <select name="livello">	<option value="Principiante">Principiante</option>	<option value="Intermedio">Intermedio</option>	<option value="Avanzato">Avanzato</option></select></td></tr>';
			...
			echo '<tr><td><input name="crea" type="submit" value="Crea nuovo corso"/></td>';
			echo '</form><td><form action="gcorsi.php"><button >Annulla</button></form></td></tr></table>';
		...
		$result = $conn->query($sql) or die("Errore nella query MySQL: ".$conn->error);
		if ($result->num_rows > 0) {
				echo '<table width ="100%" border="0" align="center" cellpadding="5" cellspacing="5"><tr><th width="40%">Nome del Corso</th><th width="20%">Livello Corso</th><th width="20%">Istruttore</th><th width="20%"></th><th width="20%"></th></tr>';
				while($row = $result->fetch_assoc()) {					
